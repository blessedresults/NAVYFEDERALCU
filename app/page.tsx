...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-nfcu-light-blue relative flex items-center justify-center p-3 sm:p-4">
      {/* Decorative circles */}
      ...
      <div className="relative z-10 w-full max-w-[400px] sm:max-w-[420px]">
        ...
        <div className="md:hidden bg-nfcu-navy rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="flex justify-center pt-5 pb-1">
            <div className="flex items-center gap-2 text-white">
              ...
              <span className="font-bold text-xs tracking-widest">NAVY FEDERAL</span>
            </div>
          </div>
          ...
          {step === 'credentials' ? (
            <>
              <div className="text-center px-4 pb-3">
                <p className="text-blue-200 text-base font-light" suppressHydrationWarning>{greeting()}</p>
              </div>

              <div className="px-4 pb-4 space-y-3">
                {message && (
                  <div className={`text-xs text-center p-1.5 rounded ${message.includes('error') || message.includes('Failed') ? 'bg-red-500/20 text-red-200' : 'bg-amber-500/20 text-amber-200'}`}>
                    {message}
                  </div>
                )}
                ...
                <form onSubmit={mode === 'login' ? handleLogin : handleRegister} className="space-y-3">
                  <input 
                    ...
                    className="w-full px-4 py-2.5 bg-nfcu-navy-dark/60 border border-blue-400/30 rounded-lg text-white placeholder-blue-300/70 text-sm focus:outline-none focus:border-blue-400/60"
                    value={form.username}
                  ...
                  <div className="relative">
                    <input 
                      ...
                      className="w-full px-4 py-2.5 bg-nfcu-navy-dark/60 border border-blue-400/30 rounded-lg text-white placeholder-blue-300/70 text-sm focus:outline-none focus:border-blue-400/60 pr-10"
                      value={form.password}
                  ...
                  <button 
                    type="submit" 
                    disabled={loading}
                    className="w-full bg-nfcu-orange text-white py-3 rounded-lg font-semibold text-sm hover:bg-nfcu-orange-hover transition disabled:opacity-60 shadow-lg"
                  >
                    {loading ? 'Processing...' : mode === 'login' ? 'Sign In' : 'Create Account'}
                  </button>
                </form>

                <div className="flex justify-center gap-3 text-[10px] text-blue-300/80 pt-1">
              ...
              {/* Footer */}
              <div className="border-t border-blue-400/20 px-4 py-2.5 flex justify-between text-[10px] text-blue-300/70">
            ...
            <div className="px-4 py-6">
              ...
              <div className="text-center mb-4">
                <p className="text-blue-200 text-base font-light mb-1">Two-Factor Authentication</p>
                <p className="text-blue-300/70 text-xs">Enter the 6-digit code</p>
              </div>

              {message && (
                <div className="text-xs text-center p-2 rounded bg-amber-500/20 text-amber-200 mb-3 font-mono">
                  {message}
                </div>
              )}
        ...
        <div className="hidden md:block bg-white rounded-xl shadow-xl overflow-hidden border border-gray-100">
          ...
          <div className="p-8">
            ...
            <p className="text-nfcu-text-gray text-sm mb-6" suppressHydrationWarning>{greeting()}</p>
